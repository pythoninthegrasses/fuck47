...
This feature will transform the existing static landing page into a dynamic news aggregation site that displays current events involving DJT with negative sentiment analysis. The site will maintain its existing visual identity while adding real-time news content filtered for negative sentiment, displayed through a cartoon DJT character with speech bubble interface.

**Problem it solves:** Users want to see current negative news coverage about DJT in a visually engaging format without navigating to multiple news sources.

**Goal:** Create an automated news aggregation system that displays DJT-related negative news stories in an entertaining, cartoon-style interface.

## Goals

1. Automatically aggregate DJT-related news from multiple sources (NewsAPI + RSS feeds)
2. Filter and display only articles with negative sentiment
3. Present content through a cartoon DJT character with speech bubble interface
...
- **As a visitor**, I want to see current negative news about DJT so that I can stay informed about controversial developments
...
1. **News Aggregation System**
   ...
   - Must search for DJT-related keywords ("Donald Trump", "Trump", "45th President", "47th President")
   - Must store and manage API keys securely via environment variables

2. **Content Filtering & Sentiment Analysis**
   - Must pre-filter news sources known for negative DJT coverage
...
4. **Visual Presentation**
   - Must position cartoon DJT image in bottom-left corner of page
...
- **Basecoat Integration**: Use Basecoat's card, button, and typography components for news display
- **Visual Hierarchy**: Speech bubble should be prominent and centered, with DJT cartoon as supporting visual element
...
- **Sentiment Accuracy**: 90%+ of displayed articles have genuinely negative sentiment about DJT
...
6. **Mobile Experience**: Should the cartoon DJT and speech bubble layout adapt differently for mobile vs desktop?
